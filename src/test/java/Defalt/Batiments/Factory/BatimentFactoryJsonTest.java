...
class BatimentFactoryJsonTest {
	...
	void testJsonToBatiments() {
		try {
			...
			assertEquals(batiments.size(), batimentsDeserialises.size(),
					"Le nombre de bâtiments désérialisés est incorrect.");
			assertEquals(batiments.get(0).getNom(), batimentsDeserialises.get(0).getNom(),
					"Le nom du premier bâtiment est incorrect.");
			assertEquals(batiments.get(1).getUsage(), batimentsDeserialises.get(1).getUsage(),
					"L'usage du deuxième bâtiment est incorrect.");
		} catch (Exception e) {
	...
	void testJsonToBatimentsFail() {
		try {
			...
			// Vérification des données désérialisées
			assertNotNull(batimentsDeserialises, "La liste désérialisée est null.");
			assertEquals(batiments.size(), batimentsDeserialises.size() - 2,
					"Le nombre de bâtiments désérialisés est incorrect.");
			assertNotEquals(batiments.get(0).getNom(), batimentsDeserialises.get(0).getNom(),
					"Le nom du premier bâtiment est incorrect.");
			assertNotEquals(batiments.get(1).getUsage(), batimentsDeserialises.get(1).getUsage(),
					"L'usage du deuxième bâtiment est incorrect.");
		} catch (Exception e) {
			fail("La méthode jsonToBatiments a levé une exception : " + e.getMessage());
		}

...
/**
 * Classe représentant un campus contenant plusieurs bâtiments. Fournit des
 * fonctionnalités pour créer, gérer, et vérifier les bâtiments.
 */
public class Campus implements Observable {
	 ...
	 */
	public void createBatiment(String nom, String usage, int surfacePiece, boolean startOne, int nbBureau, int nbEtage,
			int nbPiece) {
		Batiment batiment = batimentFactory.createBatiment(nom, usage, surfacePiece, startOne, nbBureau, nbEtage,
				nbPiece);
	...
	public boolean destroyBatiment(String nom) {
		return batiments.stream().filter(batiment -> batiment.getNom().equals(nom)).findFirst().map(batiment -> {
	...
	public boolean updateNomBatiment(String oldNom, String newNom) {
		return batiments.stream().filter(batiment -> batiment.getNom().equals(oldNom)).findFirst().map(batiment -> {
	...
	public boolean updatePieceEstBureau(String nom, int numPiece) {
		Batiment batiment = batiments.stream().filter(b -> b.getNom().equals(nom)).findFirst().orElse(null);

		if (batiment == null)
			return false;

		return batiment.getPieces().stream().filter(piece -> piece.getNumero() == numPiece).findFirst().map(piece -> {
	...
	/**
	 * Importe des bâtiments depuis un fichier JSON et les ajoute au campus si
	 * valides.
	 *
	...
	public String jsonToBatiments(String nomFichierEntree) throws Exception {
		BatimentFactoryJson factoryJson = new BatimentFactoryJson();
		List<Batiment> batimentsImported = factoryJson.jsonToBatiments(nomFichierEntree).stream()
	...
	/**
	 * Vérifie les problèmes associés à un bâtiment donné et retourne une
	 * description des erreurs.
	 *
	 * @param listProbleme   une liste des problèmes détectés pour le bâtiment.
	 * @param batiment       le bâtiment à vérifier.
	 * @param numeroBatiment l'identifiant ou l'index du bâtiment (utilisé si le
	 *                       bâtiment est null ou ne possède pas de nom valide).
	...
	public String checkError(List<ProblemeBatiment> listProbleme, Batiment batiment, int numeroBatiment) {
		...
		for (ProblemeBatiment problemeBatiment : listProbleme) {
			...
			case USAGE:
				...
				break;
			case ETAGES:
				res.append("    L'ordre des étages est incorrecte ").append(problemeBatiment.getEtagesProblemes())
						.append("\n");
				break;
			case PIECES:
				res.append("    L'ordre des pieces est incorrecte ").append(problemeBatiment.getPiecesProblemes())
						.append("\n");
				break;
			case NULLETAGE:
	...
	/**
	 * Affiche les détails d'un bâtiment en utilisant un visiteur et les encapsule
	 * dans un objet TreeItem.
	 *
	 * @param nomBatiment le nom du bâtiment dont on souhaite afficher les détails.
	 * @param visiteur    le visiteur utilisé pour visiter le bâtiment et récupérer
	 *                    ses informations.
	 * @return un objet TreeItem contenant les détails du bâtiment, ou null si le
	 *         bâtiment n'existe pas.
	 */
	public TreeItem<String> afficherDetailsBatiment(String nomBatiment, Visiteur visiteur) {
		Batiment batiment = batiments.stream().filter(b -> b.getNom().equals(nomBatiment)).findFirst().orElse(null);
		if (batiment == null) {
			return null;
		}
	 ...
	 *
	 * @param fileName le nom du fichier dans lequel les bâtiments seront
	 *                 sérialisés.
	...
	/**
	 * Désérialise une liste de bâtiments depuis un fichier et traite les éventuels
	 * problèmes associés.
	 *
	 * @param fileName le nom du fichier contenant les bâtiments sérialisés.
	 * @return une chaîne de caractères contenant les erreurs rencontrées pour
	 *         chaque bâtiment.

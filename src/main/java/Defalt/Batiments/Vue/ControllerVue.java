 ...
 * de l'application, comme la liste des bâtiments et les détails d'un bâtiment
 * spécifique. Elle utilise JavaFX pour afficher les vues et gère les
...
public class ControllerVue extends Application {
	...
	/**
	 * Objet {@link Campus} représentant le modèle principal de l'application. Il
	 * contient les bâtiments et permet d'appliquer le pattern observateur pour la
	...
	/**
	 * Point d'entrée de l'application JavaFX. Initialise le modèle et affiche la
	 * vue principale de la liste des bâtiments.
	 *
	...
	public void ouvertureVueListeBatiments(Stage primaryStage) {
		...
		try {
			...
			vueListeBatiments.getMenuItemAfficherDetailsBatiment().setOnAction(event -> {
				String selectedItem = (String) vueListeBatiments.getListViewBatiments().getSelectionModel()
						.getSelectedItem();
	...
	public void ouvertureVueBatiment(String nomBatiment) {
		...
		boolean vueExist = listVueBatiments.stream().map(VueBatiment::getNomBatiment).toList().contains(nomBatiment);
